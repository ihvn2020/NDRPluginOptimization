...
public class LabDictionary {
    ...
    public LaboratoryReportType createLaboratoryOrderAndResult(Patient pts, Encounter enc, List<Obs> labObsList, Date artStartDate)
        ...
        if(!isUtilSingletonInitiated){
            UtilsSingleton.init(pts,labObsList);
        }
        ...
        try {
            ...
            XMLGregorianCalendar convertedDate = UtilsCustom.getXmlDate(enc.getEncounterDatetime());
            labReportType.setVisitID(UtilsCustom.getVisitId(pts, enc));
            labReportType.setVisitDate(convertedDate);
            labReportType.setCollectionDate(convertedDate);

            // Date artStartDate = UtilsCustom.extractARTStartDate(pts,labObsList);
...
//            Obs obs = extractObs(Visit_Type_Concept_Id, labObsList);
            Obs obs = UtilsCustom.extractObs(Visit_Type_Concept_Id, labObsList);
            ...
            obs = UtilsCustom.extractObs(Laboratory_Identifier_Concept_Id, labObsList);
    ...
    public List<LaboratoryReportType> createLaboratoryOrderAndResult(Patient pts, List<Encounter> allPatientEncounterList, List<Obs> allPatientObsList) {
        ...
        Integer[] encounterTypeArr = {UtilsCustom.LAB_ORDER_AND_RESULT_ENCOUNTER_TYPE};

        Set<Date> visitDateSet = UtilsCustom.extractUniqueVisitsForEncounterTypes(pts, allPatientEncounterList, encounterTypeArr);
        List<Obs> obsForVisit = null;
        LaboratoryReportType labReportType = null;
        for (Date date : visitDateSet) {
            obsForVisit = UtilsCustom.extractObsPerVisitDate(date, allPatientObsList);
...
OtherLaboratoryInformation
    ...
    private List<LaboratoryOrderAndResult> createLaboratoryOrderAndResult(Encounter enc, List<Obs> obsList)
        ...
        Obs obsEle = UtilsCustom.extractObs(Ordered_Date_Concept_id, obsList);
        if (obsEle != null) {
