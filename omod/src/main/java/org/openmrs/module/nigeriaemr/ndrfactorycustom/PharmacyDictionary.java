...
import org.openmrs.module.nigeriaemr.ndrUtils.UtilsCustom;
import org.openmrs.module.nigeriaemr.ndrUtils.UtilsSingleton;

import javax.xml.datatype.DatatypeConfigurationException;
import java.util.*;

import static org.openmrs.module.nigeriaemr.ndrUtils.UtilsCustom.getXmlDate;
import static org.openmrs.module.nigeriaemr.ndrUtils.UtilsCustom.isUtilSingletonInitiated;
...
public class PharmacyDictionary {
 ...
 /*regimenMap.put(165257, "CTX480");
    ...
    private static void initializeSingleton(Patient patient,  List<Obs> obs){
        if(!isUtilSingletonInitiated){
            UtilsSingleton.init(patient,obs);
        }
    }
    ...
    public List<RegimenType> createRegimenTypeList(Patient patient, List<Encounter> allEncounterForPatient, List<Obs> patientsObsList) throws DatatypeConfigurationException {
       // Instantiate Singleton
        initializeSingleton(patient,patientsObsList);

        List<RegimenType> regimenTypeList=new ArrayList<RegimenType>();
        Integer[] targetEncounterTypes={UtilsCustom.CARE_CARD_ENCOUNTER_TYPE,Utils.PHARMACY_ENCOUNTER_TYPE};
        RegimenType regimenType=null;
        List<Obs> obsPerVisit=null;
        Set<Date> visitDateSet=UtilsCustom.extractUniqueVisitsForEncounterTypes(patient, allEncounterForPatient, targetEncounterTypes);
        for(Date visitDate: visitDateSet){
            obsPerVisit=UtilsCustom.extractObsPerVisitDate(visitDate, patientsObsList);
    ...
    public List<RegimenType> createOIRegimenTypeList(Patient patient, List<Encounter> allEncounterForPatient,List<Obs> patientsObsList) throws DatatypeConfigurationException{
        ...
        for(Encounter enc: allEncounterForPatient){
            if(enc.getEncounterType().getEncounterTypeId()==UtilsCustom.PHARMACY_ENCOUNTER_TYPE){
  ...
  -ReasonRegimenEndedCode
        ...
        RegimenType regimenType = null;
        PatientIdentifier pepfarIdentifier = patient.getPatientIdentifier(UtilsCustom.PEPFAR_IDENTIFIER_INDEX);
        String pepfarID = "";
        String ndrCode = "";
        Obs obs = null;
        int valueCoded = 0, durationInDays = 0;
        CodedSimpleType codedSimpleType = null;
        if (!obsListForAVisit.isEmpty() && pepfarIdentifier != null && UtilsCustom.contains(obsListForAVisit, UtilsCustom.CURRENT_REGIMEN_LINE_CONCEPT)) {
            pepfarID = pepfarIdentifier.getIdentifier();

            visitID = UtilsCustom.getVisitId(pepfarID, visitDate);

            regimenType = new RegimenType();
            regimenType.setVisitID(visitID);
            regimenType.setVisitDate(getXmlDate(visitDate));
            obs = UtilsCustom.extractObs(UtilsCustom.VISIT_TYPE_CONCEPT, obsListForAVisit);//PrescribedRegimenInitialIndicator
            if (obs != null && obs.getValueCoded() != null) {
                valueCoded = obs.getValueCoded().getConceptId();
                if (valueCoded == UtilsCustom.VISIT_TYPE_INITIAL_CONCEPT) {
            ...
            obs = UtilsCustom.extractObs(UtilsCustom.CURRENT_REGIMEN_LINE_CONCEPT, obsListForAVisit); //PrescribedRegimenLineCode
            if (obs != null && obs.getValueCoded() != null) {
                valueCoded = obs.getValueCoded().getConceptId();
                ndrCode = getRegimenMapValue(valueCoded);
                regimenType.setPrescribedRegimenLineCode(ndrCode);
                regimenType.setPrescribedRegimenTypeCode(UtilsCustom.ART_CODE);
                obs = UtilsCustom.extractObs(valueCoded, obsListForAVisit); // PrescribedRegimen
                if (obs != null) {
                    valueCoded = obs.getValueCoded().getConceptId();
                    ndrCode = getRegimenMapValue(valueCoded);
                ...
                if (stopDateTime != null) {
                    durationInDays = UtilsCustom.getDateDiffInDays(startDateTime.toDate(), stopDateTime.toDate());
                    ...
                    regimenType.setDateRegimenEndedDD(UtilsCustom.getDayDD(stopDate));
                    regimenType.setDateRegimenEndedMM(UtilsCustom.getMonthMM(stopDate));
                    regimenType.setDateRegimenEndedYYYY(UtilsCustom.getYearYYYY(stopDate));
            ...
            obs = UtilsCustom.extractObs(UtilsCustom.REASON_FOR_REGIMEN_SUBSTITUTION_OR_SWITCH_CONCEPT, obsListForAVisit);//ReasonForRegimenSwitchSubs
            if (obs != null && obs.getValueCoded() != null) {
                valueCoded = obs.getValueCoded().getConceptId();
                ndrCode = getRegimenMapValue(valueCoded);
            ...
            regimenType.setDateRegimenStartedDD(UtilsCustom.getDayDD(visitDate));
            regimenType.setDateRegimenStartedMM(UtilsCustom.getMonthMM(visitDate));
            regimenType.setDateRegimenStartedYYYY(UtilsCustom.getYearYYYY(visitDate));

            obs = UtilsCustom.extractObs(UtilsCustom.NUMBER_OF_MISSED_DOSES_PER_MONTH_CONCEPT, obsListForAVisit);
            if (obs != null && obs.getValueCoded() != null) {
                valueCoded = obs.getValueCoded().getConceptId();
                if (valueCoded == UtilsCustom.MISSED_DOSES_FAIR_ADHERENCE_CONCEPT || valueCoded == UtilsCustom.MISSED_MEDICATION_POOR_ADHERENCE_CONCEPT) {
                    regimenType.setPoorAdherenceIndicator(Boolean.TRUE); //PoorAdherenceIndicator
                }
            } else {
                obs = UtilsCustom.extractObs(UtilsCustom.ARV_ADHERENCE_CONCEPT, obsListForAVisit);
                if (obs != null && obs.getValueCoded() != null) {
                    valueCoded = obs.getValueCoded().getConceptId();
                    if (valueCoded == UtilsCustom.ARV_ADHERENCE_FAIR_ADHERENCE_CONCEPT || valueCoded == UtilsCustom.ARV_ADHERENCE_POOR_ADHERENCE_CONCEPT) {
    ...
    public Boolean retrieveSubstitutionIndicator(List<Obs> obsList) {
        ...
        obs = UtilsCustom.extractObs(UtilsCustom.REGIMEN_MEDICATION_PLAN, obsList);
        if (obs != null && obs.getValueCoded() != null) {
            valueCoded = obs.getValueCoded().getConceptId();
            if (valueCoded == UtilsCustom.REGIMEN_MEDICATION_PLAN_SUBSTITUTE_REGIMEN_CONCEPT_VALUE) {
                ans = Boolean.TRUE;
            }
        } else {
            obs = UtilsCustom.extractObs(UtilsCustom.PICKUP_REASON_CONCEPT, obsList);
            if (obs != null && obs.getValueCoded() != null) {
                valueCoded = obs.getValueCoded().getConceptId();
                if (valueCoded == UtilsCustom.PICKUP_REASON_CONCEPT_SUBSTITUTE_VALUE) {
                    ans = Boolean.TRUE;
                }
            }
        }
    ...
    public Boolean retrieveSwitchIndicator(List<Obs> obsList) {
        ...
        if (obs != null && obs.getValueCoded() != null) {
            valueCoded = obs.getValueCoded().getConceptId();
            if (valueCoded == UtilsCustom.REGIMEN_MEDICATION_PLAN_SWITCH_REGIMEN_CONCEPT_VALUE) {
                ans = Boolean.TRUE;
            }
        } else {
            obs = UtilsCustom.extractObs(UtilsCustom.PICKUP_REASON_CONCEPT, obsList);
            if (obs != null && obs.getValueCoded() != null) {
                valueCoded = obs.getValueCoded().getConceptId();
                if (valueCoded == UtilsCustom.PICKUP_REASON_CONCEPT_SWITCH_VALUE) {
                    ans = Boolean.TRUE;
                }
            }
        }
    ...
    public DateTime retrieveMedicationDuration(Date visitDate, List<Obs> obsList) {
        ...
        Obs obsGroup = UtilsCustom.extractObs(UtilsCustom.ARV_DRUGS_GROUPING_CONCEPT_SET, obsList);
        if (obsGroup != null) {
            targetObsList.addAll(obsGroup.getGroupMembers());
            obs = UtilsCustom.extractObs(UtilsCustom.MEDICATION_DURATION_CONCEPT, targetObsList);
            if (obs != null) {
        ...
        /*if (stopDateTime == null) {
            obs = UtilsCustom.extractObs(UtilsCustom.NEXT_APPOINTMENT_DATE_CONCEPT, obsList);
            if (obs != null) {
    ...
    private RegimenType createOIType(Patient pts, Encounter enc, List<Obs> OIDrugObsList)
        ...
        try {
            ...
            regimenType.setVisitID(UtilsCustom.getVisitId(pts, enc));
            regimenType.setVisitDate(UtilsCustom.getXmlDate(enc.getEncounterDatetime()));

            //set regimen
            Obs obs = UtilsCustom.extractObs(OI_Drug_Concept_Id, OIDrugObsList);
            ...
            //set duration
            int drugDuration = 0;
            obs = UtilsCustom.extractObs(Medication_Duration_Concept_Id, OIDrugObsList);
            if (obs != null) {
            ...
            //set dispensed date
            regimenType.setPrescribedRegimenDispensedDate(UtilsCustom.getXmlDate(enc.getEncounterDatetime()));
