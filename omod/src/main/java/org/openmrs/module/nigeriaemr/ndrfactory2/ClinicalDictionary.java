...
public class ClinicalDictionary {
	...
	public final static int Weight_Concept_Id = 5089, Child_Height_Concept_Id = 5090,
	        Patient_Family_Planning_Method_Code_Concept_Id = 374, Noted_Side_Effects_Concept_Id = 159935,
	        CD4_Concept_Id = 6, CD4_Test_Date_Concept_Id = 7, Next_Appointment_Date_Concept_Id = 5096,
	        Systolic_Blood_Pressure_Concept_Id = 5085, Dystolic_Blood_Pressure_Concept_Id = 5086,
	        Pregnancy_Breastfeeding_Status_Concept_Id = 165050, Patient_Family_Planning_Concept_Id = 5271,
	        Functional_Status_Concept_Id = 165039, WHO_Clinical_Stage_Concept_Id = 5356, TB_Status_Concept_Id = 1659,
	        Other_OI_Other_Problem_Concept_Id = 160170, ARV_Drug_Adherence_Concept_Id = 165290,
	        Why_Poor_Fair_ARV_Drug_Adherence_Concept_Id = 19, Cotrimoxazole_Dose_Concept_Id = 20,
	        Cotrimoxazole_Adherence_Concept_Id = 161652, Why_Poor_Fair_Cotrimoxazole_Adherence_Concept_Id = 22,
	        INH_Dose_Concept_Id = 23, INH_Adherence_Concept_Id = 161653, Why_Poor_Fair_INH_Drug_Adherence_Concept_Id = 25;
	...
	/*
	     Completed
	 */
	public List<HIVEncounterType> createHIVEncounterType(CustomPatient patient,
		...
		try {
			...
			Integer[] encounterTypeArr = { Utils.ADULT_INITIAL_ENCOUNTER_TYPE, Utils.LAB_ORDER_AND_RESULT_ENCOUNTER_TYPE,
			        Utils.PHARMACY_ENCOUNTER_TYPE, Utils.CARE_CARD_ENCOUNTER_TYPE };
			
			Set<Date> visitDateSet = OBSUtil.getUniqueDatesForEncounterTypes(uniqueEncounters, encounterTypeArr);//uniqueEncounterDates;//Utils.extractUniqueVisitsForEncounterTypes(patient, allPatientEncounterList,
			
			Map<Integer, List<CustomObs>> obsPerVisitDate = null;
			Date artStartDate = obsUtil.extractARTStartDate();//Utils.extractARTStartDate(patient, allPatientObsList);//Optimize for performance
			for (Date date : visitDateSet) {
				
				obsPerVisitDate = obsUtil.getAllObsForDate(date);//Utils.extractObsPerVisitDate(date, obsUtil);
				
				//System.out.println(date);
				
				if (obsPerVisitDate != null && !obsPerVisitDate.isEmpty()) {
					
					hivEncounterType = createHIVEncounterType(allPatientIdentifier, patient, date, obsPerVisitDate,
					    artStartDate, obsUtil);
	...
	private HIVEncounterType createHIVEncounterType(Map<Integer, IdentifierType> allPatientIdentifier,
	        CustomPatient patient, Date visitDate, Map<Integer, List<CustomObs>> obsListForOneVisit, Date artStartDate,
	        OBSUtil obsUtil) throws DatatypeConfigurationException {
		...
		pepfarID = allPatientIdentifier.get(Utils.PEPFAR_IDENTIFIER_INDEX).getIDNumber();//pepfarIdentifier.getIdentifier();
		...
		obs = OBSUtil.getObsFromConcept(Utils.WEIGHT_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.WEIGHT_CONCEPT).get(0);//Utils.extractObs(Utils.WEIGHT_CONCEPT, obsListForOneVisit); // Weight
		if (obs != null && obs.getValueNumeric() != 0) {
			hivEncounterType.setWeight((int) obs.getValueNumeric());
		}
		obs = OBSUtil.getObsFromConcept(Utils.CHILD_HEIGHT_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.CHILD_HEIGHT_CONCEPT).get(0); //Utils.extractObs(Utils.CHILD_HEIGHT_CONCEPT, obsListForOneVisit); // Height
		
		if (obs != null && obs.getValueNumeric() != 0) {
			System.out.println("conceptId " + obs.getConceptID());
			hivEncounterType.setChildHeight((int) obs.getValueNumeric());
		}
		CustomObs obsSystolic = null, obsDystolic = null;
		String bloodPressure = "";
		String systolicBP = "", diastolicBP = "";
		obsSystolic = OBSUtil.getObsFromConcept(Utils.BLOOD_PRESSURE_SYSTOLIC_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.BLOOD_PRESSURE_SYSTOLIC_CONCEPT).get(0);//Utils.extractObs(Utils.BLOOD_PRESSURE_SYSTOLIC_CONCEPT, obsListForOneVisit);
		obsDystolic = OBSUtil.getObsFromConcept(Utils.BLOOD_PRESSURE_DYSTOLIC_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.BLOOD_PRESSURE_DYSTOLIC_CONCEPT).get(0); //Utils.extractObs(Utils.BLOOD_PRESSURE_DYSTOLIC_CONCEPT, obsListForOneVisit);
		if (obsSystolic != null && obsSystolic.getValueNumeric() != 0 && obsDystolic != null
		        && obsDystolic.getValueNumeric() != 0) {
			systolicBP = String.valueOf(obsSystolic.getValueNumeric());
			diastolicBP = String.valueOf(obsDystolic.getValueNumeric());
			bloodPressure = systolicBP + "/" + diastolicBP;//StringUtils.join(systolicBP, "/", diastolicBP);
			hivEncounterType.setBloodPressure(bloodPressure);
		}
		obs = OBSUtil.getObsFromConcept(Utils.PREGNANCY_BREASTFEEDING_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.PREGNANCY_BREASTFEEDING_CONCEPT).get(0);//Utils.extractObs(Utils.PREGNANCY_BREASTFEEDING_CONCEPT, obsListForOneVisit);
		...
		obs = OBSUtil.getObsFromConcept(Utils.FAMILY_PLANNING_STATUS_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.FAMILY_PLANNING_STATUS_CONCEPT).get(0);//Utils.extractObs(Utils.FAMILY_PLANNING_STATUS_CONCEPT, obsListForOneVisit);
		//Does False Mean Not on Family Planninng
		if (obs != null) {
			valueBoolean = obs.isValueBoolean();
			if (valueBoolean) {
		...
		obs = OBSUtil.getObsFromConcept(Utils.FAMILY_PLANNING_METHOD_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.FAMILY_PLANNING_METHOD_CONCEPT).get(0);//Utils.extractObs(Utils.FAMILY_PLANNING_METHOD_CONCEPT, obsListForOneVisit);
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			ndrCode = getMappedValue(valueCoded);
			hivEncounterType.setPatientFamilyPlanningMethodCode(ndrCode);
		}
		obs = OBSUtil.getObsFromConcept(Utils.FUNCTIONAL_STATUS_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.FUNCTIONAL_STATUS_CONCEPT).get(0);//Utils.extractObs(Utils.FUNCTIONAL_STATUS_CONCEPT, obsListForOneVisit);
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			ndrCode = getMappedValue(valueCoded);
			hivEncounterType.setFunctionalStatus(ndrCode);
		}
		obs = OBSUtil.getObsFromConcept(Utils.WHO_CLINICAL_STAGE_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.WHO_CLINICAL_STAGE_CONCEPT).get(0);//Utils.extractObs(Utils.WHO_CLINICAL_STAGE_CONCEPT, obsListForOneVisit);
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			ndrCode = getMappedValue(valueCoded);
			hivEncounterType.setWHOClinicalStage(ndrCode);
		}
		obs = OBSUtil.getObsFromConcept(Utils.TB_STATUS_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.TB_STATUS_CONCEPT).get(0);//Utils.extractObs(Utils.TB_STATUS_CONCEPT, obsListForOneVisit);
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			ndrCode = getMappedValue(valueCoded);
		...
		obsL = OBSUtil.getObsListFromConcept(Utils.OTHER_OI_OTHER_PROBLEMS, obsListForOneVisit);//obsListForOneVisit.get(Utils.OTHER_OI_OTHER_PROBLEMS);//Utils.extractObsList(Utils.OTHER_OI_OTHER_PROBLEMS, obsListForOneVisit);
		if (obsL != null && !obsL.isEmpty()) {
			for (CustomObs ob : obsL) {
				if (ob != null && ob.getValueCoded() != 0) {
					valueCoded = ob.getValueCoded();
		...
		obsL = OBSUtil.getObsListFromConcept(Utils.NOTED_SIDE_EFFECT_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.NOTED_SIDE_EFFECT_CONCEPT);//Utils.extractObsList(Utils.NOTED_SIDE_EFFECT_CONCEPT, obsListForOneVisit);
		if (obsL != null && !obsL.isEmpty()) {
			for (CustomObs ob : obsL) {
				if (ob != null && ob.getValueCoded() != 0) {
		...
		obs = OBSUtil.getObsFromConcept(Utils.CURRENT_REGIMEN_LINE_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.CURRENT_REGIMEN_LINE_CONCEPT).get(0);//Utils.extractObs(Utils.CURRENT_REGIMEN_LINE_CONCEPT, obsListForOneVisit);
		String regimenName = "";
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			obs = OBSUtil.getObsFromConcept(valueCoded, obsListForOneVisit);//Utils.extractObs(valueCoded, obsListForOneVisit);
			if (obs != null && obs.getValueCoded() != 0) {
				valueCoded = obs.getValueCoded();
		...
		CustomObs oiDrugNameObs = null, oiStrengthObs = null;
		//Get the drugs that was entered for the day
		obs = OBSUtil.getObsFromConcept(Utils.OI_DRUGS_GROUPING_CONCEPT_SET, obsListForOneVisit);//obsListForOneVisit.get(Utils.OI_DRUGS_GROUPING_CONCEPT_SET).get(0);//Utils.extractObs(Utils.OI_DRUGS_GROUPING_CONCEPT_SET, obsListForOneVisit);
		
		if (obs != null) {
			
			//Set<Obs> obsGroupMembersSet = obs.getGroupMembers();
			//Map<Integer, CustomObs> obsGroupMembersList = obsUtil.getObsForGroup(obs.getObsGroupID());//new ArrayList<Obs>(obsGroupMembersSet);
			List<CustomObs> obsGroupMembersList = obsUtil.getObsGroup(obs.getObsGroupID());
			
			//System.out.println("Obs group size "+obsGroupMembersList.size());
			oiDrugNameObs = OBSUtil.extractObsByValuesForMap(Utils.OI_DRUGS_CONCEPT, Utils.COTRIMOXAZOLE_DRUG_CONCEPT,
			    obsListForOneVisit); //Utils.extractObsByValues(Utils.OI_DRUGS_CONCEPT, Utils.COTRIMOXAZOLE_DRUG_CONCEPT,obsGroupMembersList);
			// What to do if they select wrong strength for CTX
			
			if (oiDrugNameObs != null) {
				
				oiStrengthObs = OBSUtil.getObsFromConcept(Utils.ARV_DRUG_STRENGTH_CONCEPT, obsListForOneVisit);//obsGroupMembersList.get(Utils.ARV_DRUG_STRENGTH_CONCEPT);//(Utils.ARV_DRUG_STRENGTH_CONCEPT, obsGroupMembersList);
				if (oiStrengthObs != null && oiStrengthObs.getValueCoded() != 0) {
					valueCoded = oiStrengthObs.getValueCoded();
		...
		obs = OBSUtil.getObsFromConcept(Utils.COTRIMOXAZOLE_ADHERENCE_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.COTRIMOXAZOLE_ADHERENCE_CONCEPT).get(0);//Utils.extractObs(Utils.COTRIMOXAZOLE_ADHERENCE_CONCEPT, obsListForOneVisit);
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			ndrCode = getMappedValue(valueCoded);
			hivEncounterType.setCotrimoxazoleAdherence(ndrCode);
		}
		obs = OBSUtil.getObsFromConcept(Utils.INH_ADHERENCE_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.INH_ADHERENCE_CONCEPT).get(0);//Utils.extractObs(Utils.INH_ADHERENCE_CONCEPT, obsListForOneVisit);
		if (obs != null && obs.getValueCoded() != 0) {
			valueCoded = obs.getValueCoded();
			ndrCode = getMappedValue(valueCoded);
			hivEncounterType.setINHAdherence(ndrCode);
		}
		obs = OBSUtil.getObsFromConcept(Utils.CD4_COUNT_CONCEPT, obsListForOneVisit);//obsListForOneVisit.get(Utils.CD4_COUNT_CONCEPT).get(0);///Utils.extractObs(Utils.CD4_COUNT_CONCEPT, obsListForOneVisit);
		int cd4Count = 0;
		if (obs != null && obs.getValueNumeric() != 0) {
			cd4Count = (int) obs.getValueNumeric();
		...
		nextAppointmentDate = obsUtil.extractMedicationDuration2(visitDate);//OBSUtil.extractMedicationDuration(visitDate, obsListForOneVisit);
	...
	public HIVEncounterType createHIVEncounterType(Patient patient, Encounter enc, List<Obs> obsList)
		...
		if (dateOfLastReport != null && (dateOfLastReport.after(artStartDate) || dateOfLastReport.equals(artStartDate))) {
			...
			Period period = Period.between(new java.sql.Date(dateOfLastReport.getTime()).toLocalDate(), new java.sql.Date(
			        EnrollmentDate.getTime()).toLocalDate());
				...
				switch (conceptID) {
					...
					case Patient_Family_Planning_Concept_Id:
						...
						if (obs.getValueBoolean() != null) {
							hivEncounter
							        .setPatientFamilyPlanningCode(resolvePatientFamilyPlanningCode(obs.getValueBoolean()));
						}
