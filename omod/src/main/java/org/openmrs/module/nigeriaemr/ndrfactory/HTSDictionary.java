...
public class HTSDictionary {
	...
	private Map<Integer, String> htsDictionary;
	
	private Map<Integer, Boolean> htsBooleanDictionary;
	
	private Map<Integer, YNCodeType> htsYNCodeTypeDict;
	
	private void loadDictionary() {
        //Map OpenMRS concepts to corresponding NDR values
        htsDictionary= new HashMap<>();
	...
	private void loadBooleanDictionary() {
        ...
        htsBooleanDictionary=new HashMap<>();
	...
	public HIVTestingReportType createClientIntakeTags(Patient patient, Encounter enc, List<Obs> allObs,
	        HIVTestingReportType hivTestingReport) throws DatatypeConfigurationException {
		...
		try {
			...
			//signature
			try {
				hivTestingReport.setCompletedBy(enc.getEncounterProviders().stream().findFirst().get().getProvider()
				        .getName());
			}
			catch (Exception ex) {
	...
	public KnowledgeAssessmentType createKnowledgeAssessmentType(Patient pts, Encounter enc, List<Obs> obsList)
		...
		if (htsIdentifier != null) {
			...
			if (obs != null && obs.getValueCoded() != null) {
				knowledgeAssessmentType.setClientInformedAboutHIVTransmissionRoutes(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
			obs = extractObs(Client_Informed_About_Risk_Factors_Concept_Id, obsList);
			if (obs != null && obs.getValueCoded() != null) {
				knowledgeAssessmentType.setClientInformedOfHIVTransmissionRiskFactors(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
	...
	public HIVRiskAssessmentType createHivRiskAssessment(Patient pts, Encounter enc, List<Obs> obsList)
	        throws DatatypeConfigurationException {
		PatientIdentifier htsIdentifier = pts.getPatientIdentifier(ConstantsUtil.HTS_IDENTIFIER_INDEX);
		...
		if (htsIdentifier != null) {
			...
			if (obs != null && obs.getValueCoded() != null) {
				hivRiskAssessmentType.setUnprotectedSexWithCasualPartnerinLast3Months(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
			obs = extractObs(Unprotected_Sex_With_Regular_Partner_In_Last_3_Months, obsList);
			if (obs != null && obs.getValueCoded() != null) {
				hivRiskAssessmentType.setUnprotectedSexWithRegularPartnerInLast3Months(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
	...
	public SyndromicSTIScreeningType createSyndromicsStiType(Patient pts, Encounter enc, List<Obs> obsList)
	        throws DatatypeConfigurationException {
		PatientIdentifier htsIdentifier = pts.getPatientIdentifier(ConstantsUtil.HTS_IDENTIFIER_INDEX);
		...
		if (htsIdentifier != null) {
			...
			if (obs != null && obs.getValueCoded() != null) {
				...
				syndromicSTIScreeningType.setVaginalDischargeOrBurningWhenUrinating(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			...
			if (obs != null && obs.getValueCoded() != null) {
				...
				syndromicSTIScreeningType.setLowerAbdominalPainsWithOrWithoutVaginalDischarge(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			} else {
				//handles male
			...
			if (obs != null && obs.getValueCoded() != null) {
				syndromicSTIScreeningType.setUrethralDischargeOrBurningWhenUrinating(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			} else {
				//handles female
			...
			if (obs != null && obs.getValueCoded() != null) {
				syndromicSTIScreeningType
				        .setScrotalSwellingAndPain(getBooleanMappedValue(obs.getValueCoded().getConceptId()));
			} else {
				//handles female
			...
			if (obs != null && obs.getValueCoded() != null) {
				syndromicSTIScreeningType.setGenitalSoreOrSwollenInguinalLymphNodes(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
		}
	...
	public PostTestCounsellingType createPostTestCouncellingType(Patient pts, Encounter enc, List<Obs> obsList)
	        throws DatatypeConfigurationException {
		PatientIdentifier htsIdentifier = pts.getPatientIdentifier(ConstantsUtil.HTS_IDENTIFIER_INDEX);
		...
		if (htsIdentifier != null) {
			...
			if (obs != null && obs.getValueCoded() != null) {
				postTestCounsellingType.setHIVRequestAndResultFormFilledWithCTIForm(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
			...
			if (obs != null && obs.getValueCoded() != null) {
				postTestCounsellingType
				        .setPostTestCounsellingDone(getBooleanMappedValue(obs.getValueCoded().getConceptId()));
			}
			...
			if (obs != null && obs.getValueCoded() != null) {
				postTestCounsellingType.setProvidedWithInformationOnFPandDualContraception(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
			obs = extractObs(Client_Partner_use_FP_methods, obsList);
			if (obs != null && obs.getValueCoded() != null) {
				postTestCounsellingType.setClientOrPartnerUseFPMethodsOtherThanCondoms(getBooleanMappedValue(obs
				        .getValueCoded().getConceptId()));
			}
			...
			if (obs != null && obs.getValueCoded() != null) {
				postTestCounsellingType
				        .setCondomsProvidedToClient(getBooleanMappedValue(obs.getValueCoded().getConceptId()));
			}
	...
	public ClinicalTBScreeningType createClinicalTbScreening(Patient pts, Encounter enc, List<Obs> obsList)
	        throws DatatypeConfigurationException {
		PatientIdentifier htsIdentifier = pts.getPatientIdentifier(ConstantsUtil.HTS_IDENTIFIER_INDEX);
	...
	public PartnerDetailsType createPartnerDetails(Patient pts, Encounter enc, List<Obs> obsList)
	        throws DatatypeConfigurationException {
		PatientIdentifier htsIdentifier = pts.getPatientIdentifier(ConstantsUtil.HTS_IDENTIFIER_INDEX);
	...
	public HealthFacilityVisitsType createHealthFacilityVisit(Patient pts, Encounter enc, List<Obs> obsList)
	        throws DatatypeConfigurationException {
