...
public class PharmacyDictionary {
 ...
 /*regimenMap.put(165257, "CTX480");
    ...
    public List<RegimenType> createRegimenTypeList(CustomPatient pts, OBSUtil obsUtil,  Map<Integer, List<CustomEncounter>> allPatientEncounterList, Map<Integer, Set<Date>> uniqueEncounters,  Map<Integer, IdentifierType>allPatientIdentifier) throws DatatypeConfigurationException{
        ...
        Map<Integer, List<CustomObs>> obsPerVisit=null;
        Set<Date> visitDateSet= OBSUtil.getUniqueDatesForEncounterTypes(uniqueEncounters, targetEncounterTypes);//Utils.extractUniqueVisitsForEncounterTypes(patient, allEncounterForPatient, targetEncounterTypes);
        for(Date visitDate: visitDateSet){
            obsPerVisit= obsUtil.getAllObsForDate(visitDate);//Utils.extractObsPerVisitDate(visitDate, patientsObsList);
            regimenType=createRegimenType(pts, visitDate, obsPerVisit, allPatientIdentifier, obsUtil);
            regimenTypeList.add(regimenType);
        }
        return regimenTypeList;
    }
    
    public List<RegimenType> createOIRegimenTypeList(CustomPatient pts, OBSUtil obsUtil,  Map<Integer, List<CustomEncounter>> allPatientEncounterList, Map<Integer, Set<Date>> uniqueEncounters,  Map<Integer, IdentifierType>allPatientIdentifier) throws DatatypeConfigurationException{
        List<RegimenType> regimenTypeList=new ArrayList<RegimenType>();
        Integer[] targetEncounterTypes={Utils.CARE_CARD_ENCOUNTER_TYPE,Utils.PHARMACY_ENCOUNTER_TYPE};
        RegimenType regimenType=null;
        List<Obs> obsPerVisit=null;
        /*for(CustomEncounter enc: allPatientEncounterList){
        ...
        Map<Integer, List<CustomObs>> obsList = obsUtil.getAllObsByEncounterTypeId(Utils.PHARMACY_ENCOUNTER_TYPE);
        if(obsList != null && !obsList.isEmpty())
        {
            CustomEncounter enc = allPatientEncounterList.get(Utils.PHARMACY_ENCOUNTER_TYPE).get(0);
            regimenTypeList.addAll(createOITypes(pts, enc, obsUtil, obsList));
        }
        return regimenTypeList;
     ...
     */
    public RegimenType createRegimenType(CustomPatient patient, Date visitDate, Map<Integer, List<CustomObs>> obsListForAVisit, Map<Integer, IdentifierType> allPatientIdentifiers, OBSUtil obsUtil) throws DatatypeConfigurationException {
        /*
  ...
  -ReasonRegimenEndedCode
        ...
        RegimenType regimenType = null;
        IdentifierType pepfarIdentifier = allPatientIdentifiers.get(Utils.PEPFAR_IDENTIFIER_INDEX);//patient.getPatientIdentifier(Utils.PEPFAR_IDENTIFIER_INDEX);
        String pepfarID = "";
        String ndrCode = "";
        CustomObs obs = null;
        int valueCoded = 0, durationInDays = 0;
        CodedSimpleType codedSimpleType = null;
        if (obsListForAVisit != null && pepfarIdentifier != null && obsListForAVisit.containsKey(Utils.CURRENT_REGIMEN_LINE_CONCEPT)) {
            pepfarID = pepfarIdentifier.getIDNumber();
            ...
            obs = OBSUtil.getObsFromConcept(Utils.VISIT_TYPE_CONCEPT, obsListForAVisit);//Utils.extractObs(Utils.VISIT_TYPE_CONCEPT, obsListForAVisit);//PrescribedRegimenInitialIndicator
            ...
            obs = OBSUtil.getObsFromConcept(Utils.CURRENT_REGIMEN_LINE_CONCEPT, obsListForAVisit);//Utils.extractObs(Utils.CURRENT_REGIMEN_LINE_CONCEPT, obsListForAVisit); //PrescribedRegimenLineCode
            if (obs != null && obs.getValueCoded() != 0) {
                valueCoded = obs.getValueCoded();
                ...
                obs = OBSUtil.getObsFromConcept(valueCoded, obsListForAVisit);//Utils.extractObs(valueCoded, obsListForAVisit); // PrescribedRegimen
                if (obs != null) {
                    valueCoded = obs.getValueCoded();
                    ndrCode = getRegimenMapValue(valueCoded);
                    codedSimpleType = new CodedSimpleType();
                    codedSimpleType.setCode(ndrCode);
                    codedSimpleType.setCodeDescTxt(obs.getVariableValue());
                ...
                stopDateTime = OBSUtil.extractMedicationDuration3(visitDate, obsListForAVisit, null);//obsUtil.extractMedicationDuration2(visitDate);//OBSUtil.extractMedicationDuration(visitDate, obsListForAVisit);//retrieveMedicationDuration(visitDate, obsListForAVisit, obsUtil);
            ...
            obs = OBSUtil.getObsFromConcept(Utils.REASON_FOR_REGIMEN_SUBSTITUTION_OR_SWITCH_CONCEPT, obsListForAVisit);//Utils.extractObs(Utils.REASON_FOR_REGIMEN_SUBSTITUTION_OR_SWITCH_CONCEPT, obsListForAVisit);//ReasonForRegimenSwitchSubs
            if (obs != null && obs.getValueCoded() != 0) {
                valueCoded = obs.getValueCoded();
                ndrCode = getRegimenMapValue(valueCoded);
            ...
            obs = OBSUtil.getObsFromConcept(Utils.NUMBER_OF_MISSED_DOSES_PER_MONTH_CONCEPT, obsListForAVisit);///Utils.extractObs(Utils.NUMBER_OF_MISSED_DOSES_PER_MONTH_CONCEPT, obsListForAVisit);
            if (obs != null && obs.getValueCoded() != 0) {
                valueCoded = obs.getValueCoded();
                ...
                obs = OBSUtil.getObsFromConcept(Utils.ARV_ADHERENCE_CONCEPT, obsListForAVisit);//Utils.extractObs(Utils.ARV_ADHERENCE_CONCEPT, obsListForAVisit);
                if (obs != null && obs.getValueCoded() != 0) {
                    valueCoded = obs.getValueCoded();
    ...
    public Boolean retrieveSubstitutionIndicator(Map<Integer, List<CustomObs>> obsList) {
        CustomObs obs = null;
        int valueCoded = 0;
        Boolean ans = Boolean.FALSE;
        obs = OBSUtil.getObsFromConcept(Utils.REGIMEN_MEDICATION_PLAN, obsList);//Utils.extractObs(Utils.REGIMEN_MEDICATION_PLAN, obsList);
        if (obs != null && obs.getValueCoded() != 0) {
            valueCoded = obs.getValueCoded();
            ...
            obs = OBSUtil.getObsFromConcept(Utils.PICKUP_REASON_CONCEPT, obsList);//Utils.extractObs(Utils.PICKUP_REASON_CONCEPT, obsList);
            if (obs != null && obs.getValueCoded() != 0) {
                valueCoded = obs.getValueCoded();
    ...
    public Boolean retrieveSwitchIndicator(Map<Integer, List<CustomObs>> obsList) {
        CustomObs obs = null;
        int valueCoded = 0;
        Boolean ans = Boolean.FALSE;
        obs = OBSUtil.getObsFromConcept(Utils.REGIMEN_MEDICATION_PLAN, obsList);//Utils.extractObs(Utils.REGIMEN_MEDICATION_PLAN, obsList);
    ...
    public DateTime retrieveMedicationDuration(Date visitDate, Map<Integer, List<CustomObs>> obsList, OBSUtil obsUtil) {
        DateTime stopDateTime = null;
        DateTime startDateTime = null;
        int durationDays = 0;
        CustomObs obs = null;
        Map<Integer, CustomObs> targetObsList=new HashMap<>();
        ...
        if (obsGroup != null) {
            targetObsList = obsUtil.getObsForGroup(obsGroup.getObsGroupID());
            //targetObsList.addAll(obsGroup.getGroupMembers());
            obs = targetObsList.get(Utils.MEDICATION_DURATION_CONCEPT);//Utils.extractObs(Utils.MEDICATION_DURATION_CONCEPT, targetObsList);
            if (obs != null) {
    ...
    public List<RegimenType> createOITypes(CustomPatient pts, CustomEncounter enc, OBSUtil obsUtil,  Map<Integer, List<CustomObs>> pharmacyObsList)
            throws DatatypeConfigurationException {
        ...
        /*for (Obs ele : pharmacyObsList) {// don't forget to fix this section
            try {
